...
def setup_simulation(frequency = 240):
...
def move_and_grab_cube(robot, tray_pos, table_id, plane_id, tray_id, EXCLUDE_TABLE, base_save_dir="dataset"):
    ...
    while successful_iterations < 4:
        ...
        # Spawn obstacles
        obstacle_ids_list = []
        # Obstacle between pick and place locations (y=0 to y=0.9)
        # Place 2 obstacles
        obs_configs = [
            # ([0.45, 0.4, 0.75], [0,0,0], 0.15),
            # ([0.55, 0.4, 0.75], [0,0,0], 0.15),
            ([0.25, 0.0, 0.75], [0,0,0], 0.15),
            ([0.5, 0.3, 0.75], [0,0,0], 0.25)
        ]
        
        for pos, orn, scale in obs_configs:
            obs_id = p.loadURDF("cube.urdf", pos, p.getQuaternionFromEuler(orn), globalScaling=scale)
            p.changeVisualShape(obs_id, -1, rgbaColor=[0.8, 0.2, 0.2, 1.0]) # Red obstacles
            obstacle_ids_list.append(obs_id)
            print(f"Obstacle spawned at: {pos}")
        ...
        if path:
            ...
            print("  Planning failed! Aborting attempt and restarting.")
            failed_attempts += 1
            total_attempts += 1
            p.removeBody(cube_id)
            for obs_id in obstacle_ids_list:
                p.removeBody(obs_id)
...
def main():
    ...
    tray_pos, tray_orn, table_id, plane_id, tray_id = setup_simulation(frequency = 240)
